...
//     'embed'         => true,
//     'author_name'   => 'Apos37',
//     'author_url'    => 'https://github.com/apos37',
//     'title'         => 'My title',
...
// ddtt_discord_msg( $args, $webhook );
/**
 * Send a message to our Dev Debug Tools server
 * Mention a user with <@12341234123412341>
 ...
 * @return void
 */
function ddtt_discord_msg( $args, $webhook ) {
    // Timestamp
    ...
    if ( isset( $args[ 'bot_name'] ) && $args[ 'bot_name'] != '' ) {
        $data[ 'username' ] = esc_html( 'bot_name' );
    }

    // Change bot avatar url
    if ( isset( $args[ 'bot_avatar_url'] ) && $args[ 'bot_avatar_url'] != '' ) {
        $data[ 'avatar_url' ] = esc_url( $args[ 'bot_avatar_url' ] );
    }

    // Embed
    if ( isset( $args[ 'embed' ] ) && $args[ 'embed' ] == true ) {
        $data[ 'embeds' ] = [
            [
                // Embed Type
                'type' => 'rich',
