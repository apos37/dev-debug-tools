...
<form method="get" action="<?php echo esc_url( $current_url ); ?>">
    ...
    <?php if ( ( is_multisite() && !is_network_admin() && is_main_site() ) || !is_multisite() ) { ?>
        ...
        <p>Add or remove snippets from here. <em>Note: this is still in testing with other users and works as expected on a number of sites so far, but some sites have <?php echo esc_attr( $filename ); ?> files that have been heavily updated and it may not work as expected. Therefore, please make a backup and test with caution. If you have issues with this, I encourage you to give feedback on our <a href="https://discord.gg/3HnzNEJVnR">Discord Support Server</a> so we can work on improving it for everyone.</em></p>
